...
def _skip_route_registration(method, path):
...
def _materialize_connected_video(video):
    source = video.get_stream_source()
    if isinstance(source, str) and os.path.exists(source):
        source_path = os.path.realpath(source)
        input_dir = os.path.realpath(folder_paths.get_input_directory())
        try:
            if os.path.commonpath([source_path, input_dir]) == input_dir:
                return os.path.relpath(source_path, input_dir).replace("\\", "/")
        except ValueError:
            pass
    ...
    node_id = str(get_current_node_id() or uuid.uuid4().hex)
    safe_node_id = "".join(char for char in node_id if char.isalnum() or char in "-_")
    filename = f"ic_lora_input_{safe_node_id}.mp4"
    output_path = os.path.join(output_dir, filename)
    video.save_to(output_path)
    return f"{subfolder}/{filename}"


def _inject_connected_motion_segment(tdata, video, frame_rate):
    video_file = _materialize_connected_video(video)
    try:
    ...
    length = max(1, int(math.ceil(duration * float(frame_rate)))) if duration > 0 else max(1, frame_count)
    segment = {
    ...
    motion_segments = [
        item
        for item in tdata.get("motionSegments", [])
        if item.get("source") != "ic_lora_input"
    ]
    motion_segments.insert(0, segment)
    tdata["motionSegments"] = motion_segments
    tdata["motionTrackEnabled"] = True
    return segment
...
@_skip_route_registration("GET", "/ltx_director_check_file")
...
@_skip_route_registration("GET", "/ltx_director_get_audio")
...
@_skip_route_registration("GET", "/ltx_director_open_folder")
...
@_skip_route_registration("POST", "/ltx_director_upload_chunk")
...
class LTXDirectorICInput(io.ComfyNode):
    """WYSIWYG timeline variant — segments and lengths come from a visual editor in the node UI."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="LTXDirectorICInput",
            display_name="LTX Director (IC-LoRA Input)",
            category="视频处理",
            description=(
            ...
            inputs=[
                ...
                io.Video.Input(
                    "ic_lora_input", display_name="IC-LoRA Input", optional=True,
                    tooltip="Connect a VIDEO output to automatically add it to the IC-LoRA Input timeline track.",
                ),
                io.String.Input(
    ...
    def execute(cls, model, clip, start_second, end_second, duration_seconds, start_frame, end_frame, duration_frames,
                ...
                ic_lora_input=None) -> io.NodeOutput:
        ...
        connected_segment = None
        if ic_lora_input is not None:
            try:
                connected_segment = _inject_connected_motion_segment(tdata, ic_lora_input, frame_rate)
                timeline_data = json.dumps(tdata, separators=(",", ":"))
            except Exception as e:
                log.warning("[LTXDirectorICInput] Could not map connected VIDEO input: %s", e)
        ...
        ui = {"ic_lora_input": [connected_segment]} if connected_segment else None
        return io.NodeOutput(
            patched, conditioning, latent, audio_latent, guide_data,
            motion_guide_data, float(frame_rate), audio_out, ui=ui,
        )
