...
const STYLES = `
.mmx-host{width:100%;height:100%;box-sizing:border-box;display:block}
.bd-wrap{font-family:ui-sans-serif,system-ui,-apple-system,sans-serif;color:#e0e0e0;font-size:11px;display:flex;flex-direction:column;gap:6px;width:100%;height:100%;box-sizing:border-box;position:relative;min-height:var(--comfy-widget-min-height,0px)}
...
function syncDirectorNodeSize(node, editor) {
    ...
    // r2v: keep any extra height the user dragged in — the group card stretches.
    const h = editor?.isR2vBatch?.() ? Math.max(sz[1], node.size[1]) : sz[1];
...
function bindDirectorDomWidgetSizing(node, widget, getEditor) {
    ...
    // Flexible sizing: litegraph distributes the node's free vertical space to
    // widgets via computeLayoutSize. A fixed computeSize both blocked shrinking
    // (resize clamps to computeSize) and left blank space when enlarged.
...
class MiniMaxH3DirectorEditor {
    ...
    updateDomWidgetHeight() {
        ...
        if (this.domWidget) {
            // Widget stays flexible: layout assigns computedHeight each draw
            // from computeLayoutSize (min = content, max = free node space).
            delete this.domWidget.computeSize;
    ...
    buildDOM() {
        ...
        outputBar.innerHTML = `
            ...
            <label data-i18n="output.resolution" hidden aria-hidden="true" style="display:none">输出分辨率</label>
            <select class="bd-select hidden" data-r="out-aspect" data-i18n-title="tooltip.aspectRatio" style="max-width:200px">
            ...
            <span class="bd-out-mp-wrap hidden" data-r="out-mp-wrap" data-i18n-title="tooltip.megapixels" hidden aria-hidden="true">
            ...
            <span class="bd-out-long hidden" data-r="out-long-wrap" hidden aria-hidden="true">
            ...
            <span class="bd-out-fixed" data-r="out-fixed-wrap" data-i18n-title="tooltip.customWH">
                <label data-i18n="output.width">宽度</label>
                <input type="number" class="bd-num" data-r="out-w" min="32" max="8192" step="32" value="864" style="width:56px">
                <label data-i18n="output.height">高度</label>
                <input type="number" class="bd-num" data-r="out-h" min="32" max="8192" step="32" value="480" style="width:56px">
            </span>
            <select class="bd-select hidden" data-r="out-mode" data-i18n-title="tooltip.outputMode" hidden aria-hidden="true">
    ...
    _syncR2vCardSelection() {
        ...
        cards.forEach((el, i) => {
            // r2v renders only the selected group's card, so the DOM index is
            // not the segment index — use the recorded one when available.
            const parsed = parseInt(el.dataset?.segIndex, 10);
            const segIdx = Number.isFinite(parsed) ? parsed : i;
            const runOn = !runSelectOn || this.isSegmentRunEnabled(segIdx);
            // t2v/i2v: no focus "selected" border — only run-on/run-skipped when 选择运行.
            el.classList.toggle("selected", focusSel && segIdx === this.selectedIndex);
    ...
    updateOutputModeUI() {
        // Modified: always show only width/height inputs (like LTX Director IC-LoRA Input node).
        // Resolution selector (aspect ratio dropdown), megapixels input, long-edge, and
        // output mode select are permanently hidden. Underlying timeline.output is forced
        // to fixed custom width/height so existing plan/executor logic keeps working.
        if (this.outAspect) this.outAspect.classList.add("hidden");
        if (this.outMode) this.outMode.classList.add("hidden");
        if (this.outMpWrap) this.outMpWrap.classList.add("hidden");
        if (this.outLongWrap) this.outLongWrap.classList.add("hidden");
        if (this.outFixedWrap) this.outFixedWrap.classList.remove("hidden");
        // Force custom fixed resolution mode and sync widgets.
        this.applyCustomResolution();
    }
    ...
    onMouseDown(e) {
        ...
        if (hit.type === "playhead" || hit.type === "ruler") {
            ...
            this.clearSplitSelection();
            this.updateSelectionUI();
            // r2v: the panel below shows only the selected group — rebuild it.
            if (this.isR2vBatch()) this.renderImageBatchGroups();
        ...
        for (let i = 0; i < segs.length; i++) {
            ...
            if (next >= s.start && next < s.start + s.length) {
                if (this.selectedIndex !== i) {
                    ...
                    // r2v: keep the single visible group card in sync with the playhead.
                    if (this.isR2vBatch()) this.renderImageBatchGroups();
                    this.updateSelectionUI();
                }
                break;
    ...
    updateSelectionUI() {
        this.root?.classList.toggle("bd-r2v-mode", this.isR2vBatch());
...
function isMiniMaxH3DirectorNode(node) {
    ...
    return cls === "MiniMaxH3DirectorWH";
}
...
app.registerExtension({
    name: "ComfyUI.MiniMaxH3DirectorPluginWH",
    async setup() {
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        nodeType.prototype.onResize = function (size) {
            ...
            if (!this._minimaxEditor?.isPlaying && !this._minimaxEditor?._pauseSettling) {
                // Flexible widget layout re-distributes free space every draw.
