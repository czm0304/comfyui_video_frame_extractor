...
class TimelineEditor {
  ...
  async _ensureThumbnails(seg) {
    ...
    const fileKey = seg._previewVideoFile || seg.imageFile || seg.videoFile || seg._blobUrl;
    if (!fileKey) return;
    ...
    const extractPromise = (async () => {
      ...
      const fileType = seg._previewVideoType || "input";
      const vidUrl = seg._blobUrl || (seg.videoEl ? seg.videoEl.src : null) || api.apiURL(`/view?filename=${encodeURIComponent(filename)}&type=${encodeURIComponent(fileType)}&subfolder=${encodeURIComponent(subfolder)}`);
  ...
  _ensureVideoEl(seg) {
    ...
    const cacheKey = seg._previewVideoFile || seg.imageFile || seg.videoFile || seg._blobUrl;
    if (!cacheKey) return;
    ...
    const isMotionVideo = seg.type === "motion_video" && (seg.videoFile || seg._previewVideoFile);
    if (!isVideo && !isMotionVideo) return;

    const fileKey = seg._previewVideoFile || ((seg.type === "video" || isRetake) ? seg.imageFile : seg.videoFile);
    ...
    if (!vidUrl && fileKey) {
      ...
      const fileType = seg._previewVideoType || "input";
      vidUrl = api.apiURL(`/view?filename=${encodeURIComponent(justName)}&type=${encodeURIComponent(fileType)}&subfolder=${encodeURIComponent(subfolder)}`);
    }
    if (!vidUrl) return;
  ...
  setConnectedICLoraInput(videoFile, linkId, preview = null) {
    const motionSegments = this.timeline.motionSegments || (this.timeline.motionSegments = []);
    const existingIndex = motionSegments.findIndex(seg => seg.source === "ic_lora_input");

    if (linkId === null || linkId === undefined) {
      if (existingIndex !== -1) {
        motionSegments.splice(existingIndex, 1);
        this.commitChanges(true);
        this.render();
      }
      return;
    ...
    const previewFile = preview?.filename
      ? (preview.subfolder ? `${preview.subfolder}/${preview.filename}` : preview.filename)
      : "";
    const displayName = videoFile || previewFile
      ? (videoFile || previewFile).split(/[/\\]/).pop()
      : "Connected VIDEO (run to load preview)";
    const existing = existingIndex === -1 ? null : motionSegments[existingIndex];
    if (
      existing
      && existing.connectedLinkId === linkId
      && existing.videoFile === videoFile
      && existing._previewVideoFile === previewFile
      && existing._previewVideoType === (preview?.type || "")
    ) {
      return;
    }

    const segment = existing || {
      id: "connected_ic_lora_input",
      type: "motion_video",
      source: "ic_lora_input",
      start: 0,
      trimStart: 0,
      videoStrength: 1.0,
    ...
    segment.connectedLinkId = linkId;
    segment.videoFile = videoFile || "";
    segment._previewVideoFile = previewFile;
    segment._previewVideoType = preview?.type || "";
    segment.fileName = displayName;
    segment.length = Math.max(1, segment.length || this.getVisualDurationFrames());
    segment.videoDurationFrames = Math.max(1, segment.videoDurationFrames || segment.length);
    segment._connectedPending = !videoFile;

    if (existingIndex === -1) {
      motionSegments.unshift(segment);
    }
    this.motionTrackEnabled = true;
    this.timeline.motionTrackEnabled = true;
    this.node.properties.motionTrackEnabled = true;
    this.selectionType = "motion";
    this.selectedIndex = motionSegments.indexOf(segment);

    if (videoFile || previewFile) {
      segment.videoEl = null;
      this._ensureVideoEl(segment);
      const updateLength = () => {
        if (!segment.videoEl?.duration) return;
        const clipFrames = Math.max(1, Math.ceil(segment.videoEl.duration * this.getFrameRate()));
        segment.length = clipFrames;
        segment.videoDurationFrames = clipFrames;
        this.growTimelineIfNeeded(clipFrames);
        this.commitChanges(true);
        this.updateUIFromSelection();
        this.render();
      };
      if (segment.videoEl?.readyState >= 1) {
        updateLength();
      } else {
        segment.videoEl?.addEventListener("loadedmetadata", updateLength, { once: true });
      }
    }
  ...
  commitChanges(skipRender = false) {
    ...
    const toSave = {
      ...
      motionSegments: (this.timeline.motionSegments || []).map(s => {
        const { imgObj, videoEl, _isSeeking, thumbnails, _extractingThumbs, _sSecs, _lSecs, _tSecs, _dSecs, _uploading, _blobUrl, _previewVideoFile, _previewVideoType, ...rest } = s;
        return rest;
      }),
...
app.registerExtension({
  name: "LTXDirectorICInput",
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name === "LTXDirectorICInput") {
      ...
      nodeType.prototype.onNodeCreated = function () {
        ...
        this._syncICLoraInputFromLink = function () {
          if (!self._timelineEditor) return;
          const input = self.inputs?.find(i => i.name === "ic_lora_input");
          const linkId = input?.link;
          if (linkId === null || linkId === undefined) {
            self._timelineEditor.setConnectedICLoraInput("", null);
            return;
          }
          ...
          let preview = null;
          if (originNode) {
            const preferredNames = ["file", "video", "video_file", "video_path", "filename"];
            for (const name of preferredNames) {
              const widget = originNode.widgets?.find(w => w.name === name);
              const value = widget?.value ?? originNode.properties?.[name];
              if (typeof value === "string" && value.trim()) {
                videoFile = value.trim();
                break;
              }
              if (value && typeof value === "object" && value.filename) {
                videoFile = value.subfolder
                  ? `${value.subfolder}/${value.filename}`
                  : value.filename;
                break;
              }
            }
            ...
            const getPreview = node => {
              const output = app.nodeOutputs?.[node.id] || app.nodeOutputs?.get?.(node.id);
              const item = output?.images?.[0];
              return item?.filename ? item : null;
            };
            preview = getPreview(originNode);
            if (!preview && link) {
              for (const node of app.graph._nodes || []) {
                if (node.type !== "SaveVideo") continue;
                const videoInput = node.inputs?.find(input => input.name === "video");
                const saveLink = videoInput?.link !== null && videoInput?.link !== undefined
                  ? app.graph.links[videoInput.link]
                  : null;
                if (
                  saveLink
                  && saveLink.origin_id === link.origin_id
                  && saveLink.origin_slot === link.origin_slot
                ) {
          ...
          self._timelineEditor.setConnectedICLoraInput(videoFile, linkId, preview);
        };
        ...
        this.onConnectionsChange = function (type, index, connected, link_info) {
          ...
          setTimeout(() => self._syncICLoraInputFromLink(), 0);
        };
      ...
      nodeType.prototype.onExecuted = function (message) {
        ...
        const payload = message?.ic_lora_input?.[0];
        const input = this.inputs?.find(i => i.name === "ic_lora_input");
        if (payload?.videoFile && input?.link !== null && input?.link !== undefined) {
          this._timelineEditor?.setConnectedICLoraInput(payload.videoFile, input.link);
        }
        return out;
      };
